...
<template>
	<scroll-view v-if="view" class="sk-rolecard" scroll-y>
		<view class="sk-rolecard__hero">
			...
			<view>
				<text class="sk-rolecard__name">{{ view.role.name }}</text>
		...
		<view class="sk-rolecard__block">
			<text class="sk-rolecard__label">📖 人设</text>
			<text class="sk-rolecard__text">{{ view.role.public }}</text>
		</view>

		<view class="sk-rolecard__block sk-rolecard__block--secret">
			<text class="sk-rolecard__label sk-rolecard__label--secret">🤫 秘密（仅你可见）</text>
			<text class="sk-rolecard__text">{{ view.role.secret }}</text>
		</view>

		<view class="sk-rolecard__block">
			<text class="sk-rolecard__label sk-rolecard__label--goal">🎯 你的目标</text>
			<text class="sk-rolecard__text">{{ view.role.goal }}</text>
		</view>

		<view v-if="Object.keys(view.role.relationships).length" class="sk-rolecard__block">
			<text class="sk-rolecard__label">🔗 人物关系</text>
			<view
				v-for="(rel, name) in view.role.relationships"
				:key="name"
				class="sk-rolecard__rel"
			>
				<text class="sk-rolecard__rel-name">{{ name }}</text>
				<text class="sk-rolecard__rel-text">{{ rel }}</text>
			</view>
		</view>

		<view class="sk-rolecard__block">
			<text class="sk-rolecard__label">🔑 我的线索</text>
