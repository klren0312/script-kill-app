...
<template>
	<scroll-view class="sk-clue" scroll-y>
		...
		<view class="sk-clue__section">
			<text class="sk-clue__title">🔑 我的线索（可出示）</text>
			...
			<view v-for="c in myClueTexts" :key="c.id" class="sk-clue__item">
				<text class="sk-clue__text">{{ c.text }}</text>
				<button
					v-if="canInvestigate"
					class="sk-clue__show"
					@tap="emit('show', c.id)"
		...
		<!-- 调查 -->
		<view class="sk-clue__section">
			<text class="sk-clue__title">🔍 调查某人（每回合 1 次）</text>
			<view class="sk-clue__grid">
				...
				>
					<text class="sk-clue__role-name">{{ r.name }}</text>
					<text class="sk-clue__role-state">{{ investigatedIds.includes(r.id) ? '已调查' : (canInvestigate ? '调查' : '未轮到') }}</text>
				</view>
			</view>
		</view>
	</scroll-view>
