...
<template>
	<view class="sk-msg" :class="[`sk-msg--${event.type}`, { 'sk-msg--mine': isMine }]">
		<view v-if="event.roleName && (event.type === 'speak' || event.type === 'whisper' || event.type === 'show')" class="sk-msg__name">
			{{ event.roleName }}
			<text v-if="event.type === 'whisper'" class="sk-msg__tag">私聊</text>
		</view>
		<view v-else class="sk-msg__type">{{ labelMap[event.type] || event.type }}</view>

		<view class="sk-msg__bubble">
			<template v-if="event.type === 'investigate'">
				<text class="sk-msg__text">{{ event.roleName }} 调查了 {{ event.targetName }}：</text>
				<text class="sk-msg__text sk-msg__text--hl">{{ event.text }}</text>
			</template>
			<template v-else-if="event.type === 'show'">
				<text class="sk-msg__text">{{ event.roleName }} 出示了线索：</text>
				<text class="sk-msg__text sk-msg__text--hl">{{ event.text }}</text>
			</template>
			<template v-else-if="event.type === 'vote'">
				<text class="sk-msg__text">{{ event.text || `${event.roleName} 已投票` }}</text>
			</template>
			<template v-else>
				<text class="sk-msg__text">{{ event.text }}</text>
			</template>
		</view>
	</view>
</template>
