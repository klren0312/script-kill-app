...
const winnerText = computed(() => {
  if (!props.winner)
    return ''
  if (props.winner === 'detective')
    return '🕵️ 侦探方获胜'
  if (props.winner === 'culprit')
    return '🔪 凶手方获胜'
  if (props.winner === 'abstain')
    return '⚖️ 凶手逃脱'
  return props.winner
})
</script>
  ...
  <u-popup :value="show" mode="center" :mask="true" @input="emit('close')">
    <view class="sk-result">
      <view class="sk-result__banner">
        <text class="sk-result__banner-text">
          {{ winnerText || '案件终结' }}
        </text>
      </view>
      ...
      <scroll-view v-if="truth" class="sk-result__body" scroll-y>
        <view class="sk-result__block">
          <text class="sk-result__label">
            真凶
          </text>
          <text class="sk-result__value sk-result__value--danger">
            {{ truth.culprit }}
          </text>
        </view>
        <view class="sk-result__block">
        ...
        <view class="sk-result__block">
          ...
          >
            <text class="sk-result__time">
              {{ t.time }}
            </text>
            <text class="sk-result__event">
              {{ t.event }}
            </text>
          </view>
        </view>
      ...
      <button class="sk-result__btn" @tap="emit('close')">
        合上卷宗
      </button>
    </view>
  </u-popup>
