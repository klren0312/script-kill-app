...
const currentTurnName = computed(() => {
	...
	if (!id) return '—'
	if (id === store.humanRoleId) return '你'
...
async function init() {
	...
	try {
		...
		store.applySnapshot(gRes.data)
		store.setMyRoleView(meRes.data)
		if (!store.scriptView) {
			const sRes = await getScript(gRes.data.scriptId)
			store.setScriptView(sRes.data)
		}
	}
	catch (e) {
	...
	<view class="room">
		...
		<view class="room__nav">
			<view class="room__nav-top">
				<text class="room__back" @tap="backToLobby">‹</text>
				<view class="room__nav-title">
					<text class="room__script">{{ store.scriptView?.title || '游戏房间' }}</text>
				</view>
			...
			<view class="room__status">
				...
				<text class="room__round">第 {{ store.round }}/{{ store.maxRounds }} 轮</text>
				<text class="room__turn">当前：{{ currentTurnName }}</text>
			</view>
			<view
		...
		<view class="room__tabbar">
			<view
				...
				@tap="activeTab = t.key"
			>
				<text class="room__tab-label">{{ t.label }}</text>
			</view>
		</view>
