...
async function openRole(scriptId: string) {
	try {
		...
		roleScript.value = res.data
		selectedRoleId.value = res.data.roles[0]?.id || ''
		showRole.value = true
	}
	catch {
...
async function onCreate() {
	if (!roleScript.value || !selectedRoleId.value) return
	const script = roleScript.value
	try {
		const res = await createGame({ scriptId: script.id, humanRoleId: selectedRoleId.value })
		store.saveSession(res.data.gameId, selectedRoleId.value)
		store.setScriptView(script)
		showRole.value = false
		uni.navigateTo({ url: `/pages/script-kill/room?gameId=${res.data.gameId}` })
	}
	catch (e: any) {
...
<template>
	<view class="lobby">
		...
		<view class="lobby__header">
			<text class="lobby__title">🎭 剧本杀工坊</text>
			<text class="lobby__sub">悬疑之夜，等你入局</text>
		</view>
		...
		<view class="lobby__card">
			<view class="lobby__card-head" @tap="genForm.showGenerate = !genForm.showGenerate">
				<text>✨ AI 创作新剧本</text>
				<text class="lobby__arrow">{{ genForm.showGenerate ? '收起' : '展开' }}</text>
			</view>
		<view v-if="genForm.showGenerate" class="lobby__gen">
			...
			<view class="lobby__row">
				<text class="lobby__row-label">人数</text>
				<u-number-box v-model="genForm.count" :min="3" :max="8" />
			</view>
			<view class="lobby__chips">
				<text class="lobby__chips-label">类型</text>
				<view
			...
			<view class="lobby__chips">
				<text class="lobby__chips-label">难度</text>
				<view
		...
		<view class="lobby__section">
			<text class="lobby__section-title">🔥 进行中的房间</text>
			...
			<view v-for="g in games" :key="g.id" class="lobby__room">
				<view class="lobby__room-info">
					<text class="lobby__room-name">{{ scriptName(g.scriptId) }}</text>
					<view class="lobby__room-phase">{{ phaseText(g.phase) }}</view>
				</view>
				<button class="lobby__room-join" @tap="joinGame(g)">加入</button>
			</view>
		</view>

		<!-- 剧本库 -->
		<view class="lobby__section">
			<text class="lobby__section-title">📚 剧本库</text>
		...
		<u-popup v-model="showRole" mode="bottom" :round="20">
			<view class="role-pop">
				<view class="role-pop__head">
					<text class="role-pop__title">{{ roleScript?.title }}</text>
					<text class="role-pop__close" @tap="showRole = false">✕</text>
				</view>
				...
				<button class="role-pop__btn" @tap="onCreate">进入房间</button>
			</view>
		</u-popup>
	</view>
</template>
